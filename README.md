...
Builds work for GCC 4.8, 7.2, and 9.1.  Others not fully tested but were building last time they were tried.
...
````
make GCC={4.8|4.9|5.2|7.2|9.1} REL=x.x.x SUBREL=x -jx  # I like -j32 on a 16-core server, adjust according to your CPU power
````
...
docker run --user $(id -u):$(id -g) --rm -v $(pwd):/workdir earlephilhower/gcc-cross bash -c "cd /workdir; make -j32 GCC={4.8|4.9|5.2|7.2|9.1} REL=2.5.0 SUBREL=3 all"
````

To make a draft release of the binaries:
````
make GCC={4.8|4.9|5.2|7.2|9.1} REL=2.5.0 SUBREL=3 upload
````
